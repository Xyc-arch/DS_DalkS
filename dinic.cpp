...
vector<int> Graph::Cut(int s){
    ...
    while (!q.empty())
    {
        ...
        {
            Edge &e = *i;
            if (inDS[e.v] > 0) continue;
            if (e.flow < e.C)
            {
                outIdx.push_back(e.v);
                q.push_back(e.v);
                inDS[e.v] = 1;
                // cout << "from " << u << " to " << e.v << " flow " << e.flow << endl;
            }
        }
    }
    delete[] inDS;
...
long double Graph::getCutFlow(vector<int> group){
    long double flow = 0;
    Edge ce;
    int groupMem;
    for (int i=0; i<2; i++){
        groupMem = group[i];
        for (int j=0; j<(int)(adj[groupMem].size()); j++){
            ce = adj[groupMem][j];
            if (ce.v != group[0] && ce.v != group[1]){
                flow += ce.flow;
            }
        }
    }
